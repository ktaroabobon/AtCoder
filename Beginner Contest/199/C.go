...
func changeString(stringLenHalf int, targetString string, variableInt []string) (returnString string) {
	r := []rune(targetString)
	if s2i(variableInt[0]) == 1 {
		r[s2i(variableInt[1])-1], r[s2i(variableInt[2])-1] = r[s2i(variableInt[2])-1], r[s2i(variableInt[1])-1]
	} else {
		rLatter := r[:stringLenHalf]
		r = append(r[stringLenHalf:], rLatter...)
	}
...
func main() {
	...
	for i := 0; i < queryNum; i++ {
		variableInt := ssReader()
		s = changeString(sLenHalf, s, variableInt)
	}
	fmt.Println(s)
}
...
func splitWithoutEmpty(strTargeted string) (strReturned []string) {
	strReturned = strings.Split(strTargeted, " ")
