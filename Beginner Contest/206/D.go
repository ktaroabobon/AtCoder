...
// page URL: https://atcoder.jp/contests/abc206/tasks/abc206_d
...
type ismap map[int][]int

func bfs(x int) (cnt int) {
	dist[x] = true
	q.Append(x)
...
func main() {
	...
	for i := 0; i < N/2; i++ {
		if is[i] != ris[i] {
			a, b := is[i], ris[i]
			_, f := designatedLowerBound(g[a], b)
			if !f {
				g[a] = append(g[a], b)
				g[b] = append(g[b], a)
			}
