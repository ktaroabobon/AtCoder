...
// page URL: https://atcoder.jp/contests/abc222/tasks/abc222_c
...
func main() {
	...
	for i := 0; i < 2*N; i++ {
		AS := sReader()
		tes := strings.Split(AS, "")
		p := Person{num: i + 1, win: 0, te: tes}
		ps = append(ps, p)
	}

	for i := 0; i < M; i++ {
		for j := 0; j < N; j++ {
			match(&ps[2*j], &ps[2*j+1], i)
		}
		sort.Sort(ps)
	}
