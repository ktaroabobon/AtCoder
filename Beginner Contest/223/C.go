...
// page URL: https://atcoder.jp/contests/abc223/tasks/abc223_c
...
func main() {
	...
	for i := 1; i < N+1; i++ {
		...
		s = append(s, b)
		dr[i] = dr[i-1] + a
		tr[i] = tr[i-1] + float64(a)/float64(b)
	}

	mid := tr[len(tr)-1] / 2
	idx := sort.SearchFloat64s(tr, mid) - 1

	remTime := mid - tr[idx]
	remDis := remTime * float64(s[idx])
	ans := float64(dr[idx]) + remDis
	fmt.Println(ans)
}
