...
// page URL: https://atcoder.jp/contests/abc221/tasks/abc221_c
...
func main() {
	...
	for bits := 0; bits < (1 << uint(len(is))); bits++ {
		var a, b []int
		for i := 0; i < len(is); i++ {
		...
		sa, _ := splitToString(a)
		sb, _ := splitToString(b)

		ichmax(&ans, s2i(strings.Join(sa, ""))*s2i(strings.Join(sb, "")))
	}
