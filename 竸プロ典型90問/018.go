...
// page URL: https://atcoder.jp/contests/typical90/tasks/typical90_r
...
func main() {
	T = float64(iReader())
	is := isReader()
	L, X, Y = float64(is[0]), float64(is[1]), float64(is[2])
	Q = iReader()
	for i := 0; i < Q; i++ {
		e := float64(iReader())
		ans := math.Atan2(L/2*(1-math.Cos(2*e*math.Pi/T)), math.Sqrt(math.Pow(X, 2)+math.Pow(Y+L/2*math.Sin(2*e*math.Pi/T), 2)))
		fmt.Println(ans * 180 / math.Pi)
	}
}
