...
// page URL: https://atcoder.jp/contests/typical90/tasks/typical90_g
...
func main() {
	...
	for i := 0; i < Q; i++ {
		score := math.MaxInt64
		p = iReader()
		upperCls := lowerBound(data, p)
		underCls := upperBound(data, p)

		if upperCls < N {
			ichmin(&score, iabs(p-data[upperCls]))
		}
		if underCls != -1 {
			ichmin(&score, iabs(p-data[underCls]))
		}
		fmt.Println(score)
	}
}
