...
def solve(g, tidx, T, sx, sy, down=True):
    cnt = 0
    if down:
        ny = sy - 1
        if ny >= 0 and g[ny][sx] == T[tidx]:
            if tidx == 0:
                cnt += 1
            else:
                cnt += solve(g, tidx - 1, T, sx, ny, down=True)
    nx = sx - 1
    if nx >= 0 and g[sy][nx] == T[tidx]:
        if tidx == 0:
            cnt += 1
        else:
            cnt += solve(g, tidx - 1, T, nx, sy, down=False)
    return cnt
...
def main():
    ...
    for i in range(H):
        for j in range(W):
            ...
            if g[i][j] != T[-1]:
                continue
            ans += solve(g, tl - 2, T, j, i, down=True)
